...
- `commits <N>` — 本地午夜以來的 commit 數，`N > 0` 才出現。

`(branch)` 用 MAGENTA；其餘走 KEY/VAL（灰底標籤 + 暖色數值）。沒有任何一段時整個 segment 隱藏（不設 `$REPLY`）。
...
- 所有 git 呼叫帶 `2>/dev/null`，倉庫異常（無 upstream、空倉庫等）時對應段落自動省略，不報錯。
