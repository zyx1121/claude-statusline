...
安裝一個 component 到 user layer，並掛進當前 profile。`<component>` 可以是 built-in id（如 `ctx`、`stock-ticker`）、第三方來源（git repo `owner/name`、git URL、或本機路徑）。
...
判斷來源類型：
- 純 id（`^[a-z0-9-]+$`，無 `/`、無 `.`）→ 先當 **built-in 或已存在的 user component** 處理（Step 1）。
...
3. **第三方**：built-in / user layer 都沒有時，依來源把它取到一個暫存目錄供檢視——
